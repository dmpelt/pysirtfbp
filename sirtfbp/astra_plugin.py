...
def getFilterFile(saveDir, proj_geom, nIters, reg_grad, object_size):
    ...
    :param object_size: Size of object that is reconstructed.
    :type object_size: :class:`int`
    """
    hs = ""
    ...
    hs = hs + 'object_size: ' + str(object_size) + "; "
...
class plugin(astra.plugin.ReconstructionAlgorithm2D):
    ...
    'object_size' (optional): if the scanned object is small compared to the
                              full field of view, use this parameter to set the
                              approximate size of the object in pixels. Note: it
                              is better to set this too high rather than too low,
                              setting it too low can result in major artifacts.
    'ss' (optional): supersampling to use during filter computation. Note: this
                     option is deprecated and will be ignored. It will be removed
                     in the future.
    ...
    def initialize(self, cfg, filter_dir, ss=None, reg_grad=None, object_size=None):
        ...
        if ss:
            astra.log.warn("The 'ss' options has been deprecated and will be ignored")
        self.rg = reg_grad
        if object_size:
            self.osz = int(object_size)
            if self.osz%2==0:
                self.osz += 1
        else:
            self.osz = None
